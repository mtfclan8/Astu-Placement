...
async def check_access_and_respond(msg_obj, context: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    ...
    if not subbed:
        ...
        await edit_or_reply(msg_obj, text, reply_markup=markup, parse_mode="HTML")
        return False
...
async def show_main_menu(update_or_msg, user, is_edit=False):
    ...
    if not registered:
        status_str = "<tg-emoji emoji-id=\"5420323339723881652\">⚠️</tg-emoji> Not Registered (Please Register First)"
        text = (
